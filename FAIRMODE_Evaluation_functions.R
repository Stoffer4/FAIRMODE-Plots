...
ProgramInitialization <- function(){
  ...
  ThemeAssessmentTarget <<- theme_bw() +
    theme(
  ...
  ThemeAssessmentSummary <<- ThemeAssessmentTarget +
...
FormatDELTAData <- function(Data, PolList, UsePrint){
  
  # Create a named list to store the formatted data for each specified pollutant:
  DataList        <- vector(mode = "list", length = length(PolList))
  names(DataList) <- PolList
  
  # Loop over all specified pollutants:
  for (Pol in PolList){
    
    print(paste0("Processing the pollutant: ", Pol))
    ...
    # Create a day column:
    Data2[ , date2 := as.Date(date)]
    
    DateRange <- seq.POSIXt(min(Data2$date), max(Data2$date), by = "hour") # Create a date range covering the whole period
    
    # Turn implicit missing values into explicit missing values for the whole period for each station and pollutant.
    # This is done by finding all combinations of dates and stations using CJ() and then joining this data with "Data2":
    Data2 <- as.data.table(merge(CJ(date = DateRange, Station = unique(Data2$Station)), 
                                 Data2, by = c("date", "Station"), all.x = TRUE))
    ...
    # Take appropriate averages for "O3" and take into account the data coverage for averaging:
      ...
      Data2 <- DailyMaxAvg8hO3(Data = Data2, GroupedCols = c("Station", "StationInfo"), mod, obs, date)
      
    } # End if statement over "PM10", "PM2.5", and "O3"
      
    ## Determine the data coverage of each station for the observed concentrations:
    
    DataCoverage <- Data2[ , .(DataCoverage = sum(!is.na(obs))/.N), by = Station][order(-DataCoverage)] # Data coverage
    
    if (UsePrint){
      print("DataCoverage:")
      print(head(DataCoverage, min(nrow(DataCoverage), 20)))
    }
    ...
    # Reorder columns: 
    Data2 <- Data2 %>% mutate(Country = str_sub(Station, start = 1, end = 2)) %>% # Add a country column (only makes sense for DEHM data)
      relocate(date, Station, StationInfo, obs, mod, Country) %>% ungroup()
  
    # Store the final data in a list:
    DataList[[Pol]] <- Data2
...
DailyMaxAvg8hO3 <- function(Data, GroupedCols, mod, obs, date){
...
} # End of "DailyMaxAvg8hO3()"

# Function to compute FAIRMODE statistics:
FAIRMODEAssStat <- function(Data, StartDate, EndDate, PolList){
  
  # Hourly parameters:
  alpha_H <- c(0.2, 0.79, 0.50, 0.25) # alpha**2 is the fraction of the uncertainty around the reference value which is non-proportional
  # to the concentration level
  beta_H <- 2 # Constant species-independent value chosen by FAIRMODE
  RV_H   <- c(200, 120, 25, 50) # Reference values (RV) (thresholds). Also called LV (limit value)
  U_RV_H <- c(24, 18, 36, 28)/100 # Relative measurement uncertainty estimated around a reference value (RV) for a given time averaging
  Np_H   <- c(5.2, 11, 20, 20) # Coefficient only used for seasonal/yearly averages accounting for the compensation of errors.
  # Np is the same for PM10 and PM2.5 due to insufficient data for PM2.5. It is also called "Neff"
  Nnp_H  <- c(5.5, 3, 1.5, 1.5) # Coefficient only used for seasonal/yearly averages accounting for the compensation of errors.
  ...
  # Percentile (quantile) values for all pollutants (FAIRMODE, chapter 7.1):
  Perc  <- c(0.998, 0.929, 0.901, 0.901)
  
  # Collect all uncertainty parameters and percentile (quantile) values in a data.frame:
  U_Par <- data.frame(Pol = c("NO2", "O3", "PM2.5", "PM10"),
                      alpha_H = alpha_H, beta_H = beta_H, RV_H = RV_H, U_RV_H = U_RV_H, Np_H = Np_H, Nnp_H = Nnp_H,
                      alpha_Y = alpha_Y, beta_Y = beta_Y, RV_Y = RV_Y, U_RV_Y = U_RV_Y, Np_Y = Np_Y, Nnp_Y = Nnp_Y, Perc = Perc)
  
  # Create a named list to store the formatted data for each specified pollutant:
  StatRepList        <- vector(mode = "list", length = length(PolList))
  names(StatRepList) <- PolList
  ...
  for (Pol in PolList){
    ...
    # Drop rows with missing observed or modeled concentrations:
    Data2 <- Data[[Pol]] %>% drop_na(obs, mod)
    
    # Subset data for dates (add 24 hours to include the last date):
    Data2 <- Data2[(date >= as.POSIXct(StartDate, tz = "UTC")) & (date <= (as.POSIXct(EndDate, tz = "UTC") + days(1)))]
    
    ## Number of daily exceedances per station:
    
    if (Pol %in% c("NO2", "O3", "PM10")){ # Only exceedances for these pollutants
      
      ThresholdValue <- Thresholds[ , Pol] # Current threshold value
      
      Data2[ , date2 := as.Date(date)] # Create a day column
      
      Data2[ , Exceedances := as.integer(obs > ThresholdValue)] # 1 if an exceedance is observed, 0 otherwise
      
      # 1 if at least one exceedance took place during a day:
      Exceedances <- Data2[, .(NExceedances = as.integer(sum(Exceedances) > 0)), by = .(Station, date2)]
    ...
    StationsToKeep <- unique(Data2$Station) # Stations to keep
    ...
    Data3 <- Data2 %>%
      ...
      summarize(N          = n(), # Number of measurements
                ...
                Mean_mod   = mean(mod), # Arithmetic average of modeled concentrations
                Mean_obs   = mean(obs), # Arithmetic average of observed concentrations
                ...
                # Measurement uncertainty. Depends upon "GC".
                # Janssen and Thunis (2022), Eq. 38, and DTLmanual, annex (linear relationship between U square and O square):
                RMS_U      = U_Par_tmp$U_RV_H*sqrt((1 - U_Par_tmp$alpha_H^2)*(Mean_obs^2 + Sigma_obs^2) + 
                                                     U_Par_tmp$alpha_H^2*U_Par_tmp$RV_H^2), 
                # Percentile measurement uncertainty, FAIRMODE Eq. 37 with O_i = O_perc:
                U_O_Perc   = U_Par_tmp$U_RV_H*sqrt((1 - U_Par_tmp$alpha_H^2)*O_Perc^2 + 
                                                     U_Par_tmp$alpha_H^2*U_Par_tmp$RV_H^2),
                # 95th percentile uncertainty, for yearly averaged measurements. Depends upon "GC".
                # Janssen and Thunis (2022), Eq. 39, and DTLmanual, annex:
                U_O        = U_Par_tmp$U_RV_Y*sqrt((1 - U_Par_tmp$alpha_Y^2)/U_Par_tmp$Np_Y*Mean_obs^2 + 
                                                     U_Par_tmp$alpha_Y^2*U_Par_tmp$RV_Y^2/U_Par_tmp$Nnp_Y),
                BIAS_Norm  = BIAS/(U_Par_tmp$beta_H*RMS_U), # Normalized bias
                RMSE       = sqrt(1/N*sum((obs - mod)^2)), # RMSE
                CRMSE      = sqrt(1/N*sum(((mod - Mean_mod) - (obs - Mean_obs))^2)), # CRMSE
                CRMSE_Norm = CRMSE/(U_Par_tmp$beta_H*RMS_U), # Normalized CRMSE
                MQI        = RMSE/(U_Par_tmp$beta_H*RMS_U), # Model quality indicator
                MQI_yr     = abs(Mean_obs - Mean_mod)/(U_Par_tmp$beta_Y*U_O), # MQI for year
                ...
                MPI_BIAS   = BIAS/(U_Par_tmp$beta_H*RMS_U), # MPI for bias, FAIRMODE Eq. 16, without absolute value
                MPI_R      = (1 - R)/(0.5*U_Par_tmp$beta_H^2*RMS_U^2/(Sigma_obs*Sigma_mod)), # MPI for correlation, Janssen and Thunis (2022) Eq. 17
                MPI_Sigma  = (Sigma_mod - Sigma_obs)/(U_Par_tmp$beta_H*RMS_U), # MPI for standard deviation, Janssen and Thunis (2022) Eq. 18, without absolute value
                MPI_Perc   = (M_Perc - O_Perc)/(U_Par_tmp$beta_H*U_O_Perc), # MPI for extreme events (exceedances), Janssen and Thunis (2022) Eq. 25 without absolute value
      ) %>% left_join(Exceedances, by = "Station") %>% # Join with "Exceedances" data.frame
      mutate(Country = str_sub(Station, start = 1, end = 2)) # Add a country column (only makes sense for DEHM data)
    
    NStations <- nrow(Data3) # Number of stations
    
    Data3 <- Data3 %>% mutate(CRMSE_Norm = CRMSE_Norm*SigmaDomR) # Determine if a point should be placed on the right or left side of the plot
    ...
    MQIList <- sort(Data3 %>% pull(MQI)) # Ranked MQI list
    
    # If we only have one station, MQI90 = MQI:
    if (NStations > 1) {
    ...
    NPointsMQI90 <- nrow(Data3 %>% filter(MQI > 1)) # Number of stations with MQI > 1
    
    ## MQI90 for yearly (full period) statistics:
    
    MQIList <- sort(Data3 %>% pull(MQI_yr)) # Ranked MQI list
    
    # If we only have one station, Y90 = MQI:
    if (NStations > 1) {
    ...
    NPointsY90 <- nrow(Data3 %>% filter(MQI_yr > 1)) # Number of stations with MQI_yr > 1
    
    # Rearrange some columns and limit the station name to 13 characters for better legends (same as DELTA tool)
    Data3 <- Data3 %>% relocate(c(MQI, MQI_yr), .after = N) %>%
      arrange(desc(MQI)) %>%
      mutate(Station = str_sub(Station, start = 1, end = 13))
    
    # Create a nested list with important function output:
    StatRep <- list(Data       = Data3 %>% mutate(Station = as.factor(Station), StationInfo = as.factor(StationInfo),
                                                  Country = as.factor(Country)) %>% ungroup(),
                    MQI        = list(MQI90 = MQI90, Y90 = Y90, NPoints = NStations, NPointsMQI90 = NPointsMQI90, NPointsY90 = NPointsY90),
                    Parameters = as.list(U_Par_tmp))
    
    StatRepList[[Pol]] <- StatRep # Store the nested list in "StatRepList"
    
  } # End loop over pollutants
  
  return(StatRepList) # Return statistics
  
} # End "FAIRMODEAssStat()"

# Function to create a target plot:
TargetPlotAss <- function(StatRepList, StartDate, EndDate, PolList, Version, PlotPoint, OutputDir, OutputFile, SavePlot){
  
  # Loop over all specified pollutants:
  for (Pol in PolList){
    
    StatRep <- StatRepList[[Pol]] # Subset the statistical data for the given pollutant
    ...
    lab <- paste0("\u03B1 = ", StatRep$Parameters$alpha_H, "\n",
                  "\u03B2 = ", StatRep$Parameters$beta_H, "\n",
                  "RV = ", StatRep$Parameters$RV_H, "\n",
                  "U(RV) = ", StatRep$Parameters$U_RV_H)
    ...
    PlotTitle <- paste0(Version, " assessment - ", StatRep$MQI$NPoints, " stations\nSurface ", Pol, " ", ResString, " \n", 
    ...
    # Choose the point size and a named list of "shapes" and "fills" according to "PlotPoint":
    if (PlotPoint == 1){
      
      PointSize <- 2.5
      
      # Relate the shapes according to the stations:
      ...
      # Specify a unique combination of "fill", "color", and "shape":
      Fill       <- brewer.pal(8, "Set3")[1:4] # Softer and more diverse fill colors
      Color      <- brewer.pal(8, "Dark2")[1:3] # Darker, more muted colors for contrast
      Shape      <- 21:25 # 5 "shapes"
      FillShapes <- expand_grid(Fill, Color, Shape) # Determine all unique combinations
      
      set.seed(123)  # Ensure reproducibility
      FillShapes <- FillShapes %>% slice_sample(n = 60) # Randomly sample all rows to get great colours
      
      # Relate the fills and shapes to the stations (PlotPoint == 2) or countries (PlotPoint == 3):
      FillList  <- setNames(FillShapes$Fill, unique(StatRep$Data$Station))
      ColorList <- setNames(FillShapes$Color, unique(StatRep$Data$Station))
      ShapeList <- setNames(FillShapes$Shape, unique(StatRep$Data$Station))
      
    } # End if statement over "PlotPoint"
    ...
    if (PlotPoint == 1){
      ...
      Plot <- Plot + geom_point(aes(x = CRMSE_Norm, y = BIAS_Norm, shape = Station, fill = Station, color = Station), size = PointSize) +
        scale_shape_manual(values = ShapeList, drop = FALSE) +
        scale_fill_manual(values = FillList, drop = FALSE) +
        scale_color_manual(values = ColorList, drop = FALSE)
    ...
    Plot <- Plot + annotate("text", x = -1.8, y = 0, label = "R") +
      ...
      ThemeAssessmentTarget
    
    # Add a theme:
    if (PlotPoint == 1){
      Plot <- Plot + theme(legend.position = "inside", legend.position.inside = c(1, 1), legend.justification = c("right", "top"))
    } else if (PlotPoint == 2){ # Change the legend:
      Plot <- Plot + theme(legend.position = "bottom", legend.key.size = unit(0.1, "mm"), text = element_text(size = 10),
                           legend.text = element_text(size = 3.7, margin = margin(r = 2, t = 0, l = 2, b = 0))) +
        guides(fill = guide_legend(ncol = 10), shape = guide_legend(ncol = 10))
    } 
    ...
    } # End if statement over "SavePlot"
...
SummaryReportAss <- function(StatRepList, StartDate, EndDate, PolList = Pol, Version, PointSize, OutputDir, OutputFile, SavePlot){
  ...
  for (Pol in PolList){
    ...
    Plot1 <- ggplot() +
      ...
      geom_jitter(data = anti_join(StatRep$Data, DashedZoneR, by = "Station"),
                  aes(x = Mean_obs, y = 0), color = "blue", size = PointSize, width = 0, height = 100*0.005, alpha = 0.75) +
      ...
      ThemeAssessmentSummary
    
    # Set a point in the right dashed zone if required:
    if (nrow(DashedZoneR) > 0) {
      Plot1 <- Plot1 + geom_point(aes(x = 100 + (50*max(Xrange2) - 100)/2, y = 0), color = "blue", size = PointSize, alpha = 0.75)
    }
    ...
    if (StatRep$Parameters$Pol != "PM2.5") {
      ...
      # Visualize all data points not in the dashed zones:
      Plot2 <- Plot2 + geom_jitter(data = anti_join(StatRep$Data, DashedZoneR, by = "Station"),
                                   aes(x = NExceedances, y = 0), color = "blue",
                                   size = PointSize, width = 0, height = 100*0.005, alpha = 0.75)
      
      # Set a point in the dashed zone if required:
      if (nrow(DashedZoneR) > 0) {
        Plot2 <- Plot2 + geom_point(aes(x = 100 + (50*max(Xrange2) - 100)/2, y = 0), color = "blue", size = PointSize, alpha = 0.75) # Right
      }
    ...
    Plot3 <- ggplot() +
      ...
      # Visualize all data points not in the dashed zones:
      geom_jitter(data = anti_join(anti_join(StatRep$Data, DashedZoneR, by = "Station"), DashedZoneL, by = "Station"),
                  aes(x = MPI_BIAS, y = 0), color = "blue",
                  size = PointSize, width = 0, height = 4*0.005, alpha = 0.75) +
    ...
    if (nrow(DashedZoneR) > 0) Plot3 <- Plot3 + geom_point(aes(x = 2 + (max(Xrange) - 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Right
    if (nrow(DashedZoneL) > 0) Plot3 <- Plot3 + geom_point(aes(x = -2 + (min(-Xrange) + 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Left
    ...
    Plot4 <- ggplot() +
      ...
      geom_jitter(data = anti_join(StatRep$Data, DashedZoneR, by = "Station"), aes(x = MPI_R, y = 0), color = "blue",
                  size = PointSize, width = 0, height = 2*0.005, alpha = 0.75) +
    ...
    # Set a point in the dashed zone if required:
    if (nrow(DashedZoneR) > 0) Plot4 <- Plot4 + geom_point(aes(x = 2 + (max(Xrange2) - 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Right
    ...
    Plot5 <- ggplot() +
      ...
      # Visualize all data points not in the dashed zones:
      geom_jitter(data = anti_join(anti_join(StatRep$Data, DashedZoneR, by = "Station"), DashedZoneL, by = "Station"),
                  aes(x = MPI_Sigma, y = 0), color = "blue",
                  size = PointSize, width = 0, height = 4*0.005, alpha = 0.75) +
      annotate("label", x = 2.6, y = 0, label = round(MPIBox$MPI90, 3), color = MPIBox$CircleColor) + # Colored MPI indicator on the RHS
      scale_x_continuous(breaks = seq(-2, 2, 0.5)) +
      geom_polygon(data = data.frame(x = Xrange, y = Yrange), aes(x = x, y = y), # Dashed zone, RHS of plot
    ...
    # Set a point in the dashed zones if required:
    if (nrow(DashedZoneR) > 0) Plot5 <- Plot5 + geom_point(aes(x = 2 + (max(Xrange) - 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Right
    if (nrow(DashedZoneL) > 0) Plot5 <- Plot5 + geom_point(aes(x = -2 + (min(-Xrange) + 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Left
    ...
    # Set a point in the dashed zones if required:
    if (nrow(DashedZoneR) > 0) Plot6 <- Plot6 + geom_point(aes(x = 2 + (max(Xrange) - 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Right
    if (nrow(DashedZoneL) > 0) Plot6 <- Plot6 + geom_point(aes(x = -2 + (min(-Xrange) + 2)/2, y = 0), color = "blue",
                                                           size = PointSize, alpha = 0.75) # Left
    ...
    DataSpace <- StatRep$Data %>% ungroup() %>%
      summarize(N               = n(),
                ...
                MPI_R_space     = (1 - R)/(0.5*StatRep$Parameters$beta_H^2*RMS_U^2/(Sigma_obs_space*Sigma_mod_space)), # MPI for spatial correlations, FAIRMODE Eq. 19
                MPI_Sigma_space = abs(Sigma_mod_space - Sigma_obs_space)/(StatRep$Parameters$beta_H*RMS_U)) # MPI for standard deviation, FAIRMODE Eq. 20, without absolute value
    ...
    # Set a point in the dashed zone if required:
    if (nrow(DashedZoneR) > 0) Plot7 <- Plot7 + geom_point(aes(x = 2 + (max(Xrange2) - 2)/2, y = 0), color = "blue",
                                                           size = PointSize) # Right
    ...
    PlotTitle <- paste0("               ", Version, " assessment - ", StatRep$MQI$NPoints,
